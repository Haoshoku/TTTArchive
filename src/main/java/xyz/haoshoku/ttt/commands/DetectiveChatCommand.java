...
public class DetectiveChatCommand implements CommandExecutor {
    ...
    public boolean onCommand( CommandSender sender, Command command, String s, String[] args ) {
        ...
        if ( !user.isDetective() ) {
            player.sendMessage( TTTPlugin.getPlugin().getManager().getMessageManager().getColoredValueFromKey( "ttt.command.detective_chat_only" ) );
            return true;
        }

        if ( args.length == 0 ) {
            player.sendMessage( TTTPlugin.getPlugin().getManager().getMessageManager().getColoredValueFromKey( "ttt.command.detective_chat_usage" ) );
            return true;
        }
        ...
        for ( TTTUser users : TTTUser.getUsers() ) {
            if ( users.isDetective() ) {
                users.getPlayer().sendMessage( TTTPlugin.getPlugin().getManager().getMessageManager().getColoredValueFromKey( "ttt.command.detective_chat_message" )
